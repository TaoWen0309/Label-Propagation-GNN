...
class TenGCN(nn.Module):
    def __init__(self, num_layers, num_mlp_layers, source_dim, target_dim, hidden_dim, output_dim, final_dropout, tensor_layer_type, node_pooling, PI_dim, sublevel_filtration_methods, device):
        '''
            num_layers: number of GCN layers (INCLUDING the input layer)
            num_mlp_layers: number of layers in mlps (EXCLUDING the input layer)
            source_dim: dimensionality of source input features
            target_dim: dimensionality of target input features
        ...
        self.score_node_layer = GCNConv(hidden_dim, self.num_neighbors * 2)

        # Input block, dual branches
        self.source_branch = nn.Sequential(nn.Linear(source_dim, hidden_dim),nn.ReLU())
        self.target_branch = nn.Sequential(nn.Linear(target_dim, hidden_dim),nn.ReLU())
        ...
        # output block
        self.single_attend = nn.Sequential(nn.Linear(hidden_dim, 1), nn.ReLU())
        self.dual_attend = nn.Sequential(nn.Linear(2*hidden_dim, 1), nn.ReLU())
    ...
    def compute_batch_feat(self, batch_graph, domain):
        edge_attr = None
        edge_mat_list = []
        start_idx = [0]
        pooled_x = []
    ...
    def forward(self, batch_graph, batch_PI, domain):

        Adj_block_idx, pooled_X_concat, pooled_graph_sizes = self.compute_batch_feat(batch_graph, domain)
        ...
        GCN_hidden = torch.zeros(len(graph_sizes), self.hidden_dim, self.hidden_dim, self.hidden_dim).to(self.device)
        ...
        for g_i in range(len(graph_sizes)):
            ...
            cur_node_embeddings_hidden = self.GCN_tensor_layer(cur_node_embeddings) # (n,hidden_dim,hidden_dim,hidden_dim)
            cur_graph_tensor_hideen = torch.mean(cur_node_embeddings_hidden,dim=0) # (hidden_dim,hidden_dim,hidden_dim)
            GCN_hidden[g_i] = cur_graph_tensor_hideen
            # current PI tensor
        ...
        ## output block
        # score_gcn = self.output(self.single_tensor_layer(GCN_hidden))
        # score_top = self.output(self.single_tensor_layer(PI_hidden))
        GCN_hidden = self.single_attend(GCN_hidden).squeeze()
        PI_hidden = self.single_attend(PI_hidden).squeeze()
        score_gcn = self.output(GCN_hidden)
        score_top = self.output(PI_hidden)
        ...
        batch_graph_attn = self.dual_attend(batch_graph_tensor).squeeze() # (batch_size,hidden_dim,hidden_dim)
